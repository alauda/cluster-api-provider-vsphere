...
type VSphereClusterSpec struct {
	...
	//
	// The field is set when the cluster is created or never: it cannot be added to
	// an existing cluster, and once set every field is immutable and the field
	// cannot be cleared. The endpoint, the apiserver serving certificate and the
	// guest runtime are all derived from it and cannot be changed in place.
	// +optional
