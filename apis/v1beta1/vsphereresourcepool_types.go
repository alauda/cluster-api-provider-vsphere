...
type ResourceSlotStatus struct {
	...
	// ReclaimTaskRef tracks the in-flight vCenter task for reclaiming slot resources.
	// +optional
	ReclaimTaskRef string `json:"reclaimTaskRef,omitempty"`

	// ReclaimTaskState tracks the lifecycle of the reclaim task.
	// +optional
	ReclaimTaskState string `json:"reclaimTaskState,omitempty"`

	// ReclaimVolumePath tracks the persistent disk currently being reclaimed.
	// +optional
	...
	// LastReclaimError stores the latest reclaim task failure.
	// +optional
