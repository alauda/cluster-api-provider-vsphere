...
CAPV 对齐 baremetal provider，按关联 CAPI `Cluster` 名称区分 alive 端口。
名称为 `global`（大小写不敏感）时使用 Global 策略：

| 集群类型 | `httpPort` | `httpsPort` | `extraPorts` |
| --- | ---: | ---: | --- |
| 普通集群 | `11780` | `11781` | `""` |
| `global` | `80` | `443` | `"11443 2379"` |

`apiserverPort` 不受该分支覆盖，始终取
`VSphereCluster.spec.controlPlaneLoadBalancer.port`。因此配置为 `6443` 时，
两类集群都会转发 VIP `:6443` 到控制面节点 `:6443`。

这些端口是 alive 在 VIP 上生成的 IPVS 服务，不表示 CAPV controller Pod
或节点防火墙额外开放端口。`type=external` 或未配置
`controlPlaneLoadBalancer` 时不部署 provider-managed alive，也不应用该策略。
...
| `controllers/vspherecluster_selfbuiltlb.go`（新增） | 主流程、前置对象校验、版本解析、minfo create、readiness gate、VIP probe、kube-proxy IPVS 配置、condition 设置。 |
...
8. 新增 `controllers/vspherecluster_selfbuiltlb.go`：VIP 冲突校验、kube-proxy IPVS 配置、前置对象校验、版本解析、minfo create。
...
12. 补单元测试与 envtest：bootstrap VIP cloud-config、minfo create/platform lifecycle、readiness gate、kube-proxy、webhook、创建后冻结规则（含拒绝写入空字段）。
