...
func (r resourcePoolReconciler) reconcileNormal(ctx context.Context, pool *infrav1.VSphereResourcePool) (reconcile.Result, error) {
	...
	for i := range pool.Spec.Resources {
		...
		if status.State == "Released" && status.LastReleasedTime != nil {
			...
			if status.ReclaimTaskRef != "" {
				wait, err := r.reconcileReclaimTask(ctx, pool, slot, &status)
				if err != nil {
	...
	if specChanged {
		...
		// Return immediately after spec update to avoid ResourceVersion conflict with deferred patch.
		// The next reconcile will handle status and further logic.
...
func (r resourcePoolReconciler) reclaimPhysicalResources(ctx context.Context, pool *infrav1.VSphereResourcePool, slot *infrav1.ResourceSlot, status *infrav1.ResourceSlotStatus) (bool, time.Duration, error) {
	...
	params := session.NewParams().
		...
		WithDatacenter(pool.Spec.Datacenter)
	...
	for i := range slot.PersistentDisks {
		...
		if pd.VolumePath != "" {
			log.Info("Deleting persistent disk backing for released slot", "hostname", slot.Hostname, "disk", pd.Name, "path", pd.VolumePath)
			// Delete the .vmdk file
			m := object.NewFileManager(s.Client.Client)

			// Use the datacenter from the pool spec.
			// If not provided, we cannot safely delete if multiple datacenters exist.
			dcName := pool.Spec.Datacenter
			if dcName == "" {
				return false, 0, errors.Errorf("Datacenter must be specified in VSphereResourcePool %s/%s for resource reclamation", pool.Namespace, pool.Name)
			}
			...
			status.LastReclaimError = ""
			status.RetryAfter = nil
			log.Info("Started datastore file deletion task", "hostname", slot.Hostname, "disk", pd.Name, "path", pd.VolumePath, "task", status.ReclaimTaskRef)
...
func (r resourcePoolReconciler) reconcileReclaimTask(ctx context.Context, pool *infrav1.VSphereResourcePool, slot *infrav1.ResourceSlot, status *infrav1.ResourceSlotStatus) (time.Duration, error) {
	log := ctrl.LoggerFrom(ctx)
	...
	case types.TaskInfoStateSuccess:
		log.Info("Reclaim task completed")
		for i := range slot.PersistentDisks {
