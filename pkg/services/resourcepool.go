...
// ensurePersistentDiskBootstrapData merges persistent disk cloud-config into the bootstrap secret.
func ensurePersistentDiskBootstrapData(ctx context.Context, c client.Client, bootstrapRef *corev1.ObjectReference, persistentDisks []infrav1.PersistentDisk) error {
	if bootstrapRef == nil || bootstrapRef.Kind != "Secret" || bootstrapRef.Name == "" {
		return nil
	}

	diskConfig, err := infrautil.GetPersistentDiskCloudConfig(persistentDisks)
	if err != nil {
		return err
	}
	if len(diskConfig) == 0 {
	...
	if err := c.Get(ctx, client.ObjectKey{Namespace: bootstrapRef.Namespace, Name: bootstrapRef.Name}, secret); err != nil {
		return err
	}
	if secret.Data == nil {
		return nil
	...
	merged, err := infrautil.MergeCloudConfigUserData(value, diskConfig)
	if err != nil {
		return err
	}
	if string(merged) == string(value) {
		return nil
	}

	secret.Data["value"] = merged
